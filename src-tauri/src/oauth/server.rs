    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let str = match self {
            ...
            HttpServerError::URLError(e) => {
                format!("URL Error: {}", e.to_string())
            }
            HttpServerError::HandlerError(e) => {
                format!("Internal handler error: {}", e)
            }
        };
        write!(f, "{}", str)
    }
...
pub async fn start<F, T>(addr: impl ToSocketAddrs, handler: F) -> Result<()>
where
    ...
    T: Future<Output = std::result::Result<String, String>> + Send + Sync + 'static,
{
    ...
    for req in listener.incoming() {
        match req {
            ...
            Err(err) => {
                log::error!(
                    "Error reading connect for OAuth server: {}",
                    err.to_string()
                );
            }
        }
...
async fn handle_conn<F, T>(mut stream: TcpStream, handler: F) -> Result<bool>
where
    F: Fn(String, &mut TcpStream) -> T,
    T: Future<Output = std::result::Result<String, String>> + Send + Sync + 'static,
{
    ...
    request
        .parse(&buffer)
        .map_err(|e| HttpServerError::URLError(e))?;

    let path = request.path.unwrap_or_default();

    let response = handler(path.to_string(), &mut stream)
        .await
        .map_err(|e| HttpServerError::HandlerError(e));
...
mod tests {
    ...
    async fn test_server_run() {
        start(
            ...
            |path, stream| async { Ok(("Hey, you get this?".to_string())) },
        )
        .await
        .unwrap();
    }
